...
public class Bishop extends IPiece {
	...
	public Bishop(int row, int col, String color) {
		super(row, col, color); // Llama al constructor de la superclase IPiece con la posición y el color del
								// alfil.
	}
	...
	public ArrayList<Square> getValidMoves() {
		...
		ArrayList<Square> validMoves = new ArrayList<>(); // Inicializa una lista para almacenar los movimientos válidos
															// del alfil.
		...
		for (int[] direction : directions) { // Itera sobre cada dirección posible del alfil.
			...
			while (board.isSquareValid(newRow, newColumn)) { // Mientras la nueva posición esté dentro del tablero.
				...
				if (pieceAtNewPosition == null || !pieceAtNewPosition.color.equals(this.color)) { // Verifica si la
																									...
																									// del color
																									// opuesto.
					validMoves.add(new Square(new Position(newRow, newColumn), null)); // Agrega la nueva posición a los
																						// movimientos válidos del
																						// alfil.
				}
